// https://github.com/mltframework/shotcut/blob/master/src/qml/export-edl/mlt2edl.js
/*
...
if (typeof module !== 'undefined' && module.exports) {
    ...
    if (!xmldoc)
        throw new Error(
            "Expected xmldoc to be defined. Make sure you're including xmldoc.js before this file.",
        )
    timecode = modules.timecode.Timecode
    if (!timecode)
        throw new Error(
            "Expected timecode to be defined. Make sure you're including timecode.js before this file.",
        )
}
...
function MltXmlParser(xmlString, options) {
    ...
    self.useBaseNameForReelName = self.get(
        options,
        'useBaseNameForReelName',
        true,
    )
    self.useBaseNameForClipComment = self.get(
        options,
        'useBaseNameForClipComment',
        true,
    )
    ...
    self.framerate =
        parseFloat(self.projectMeta.attr.frame_rate_num) /
        parseFloat(self.projectMeta.attr.frame_rate_den)
    return self
}
...
MltXmlParser.prototype.getTrackByProducerId = function (id) {
    ...
    this.xmldoc.childrenNamed('tractor').forEach(function (tractor) {
        ...
        if (typeof node === 'object' && node.name === 'track') result = node
    })
    return result
}
    ...
    playlists.forEach(function (p) {
        ...
        if (track && 'hide' in track.attr) {
            if (track.attr.hide === 'audio') {
                ...
                else plDict.format = 'V' // muted video track
            } else if (track.attr.hide === 'video') {
                if (plDict.format === 'V')
                    return // skip muted and hidden tracks
                else if (plDict.format.indexOf('V') !== -1) plDict.format = 'A' // hidden audio/video track
                                ...
                                tractor
                                    ...
                                    .forEach(function (track) {
                                        if (!count) {
                                            ...
                                            var length = self.Timecode(
                                                track.attr.out,
                                            )
                                            length.subtract(
                                                self.Timecode(track.attr.in),
                                            )
        ...
        playlist.events.forEach(function (event) {
            ...
            if (event.producer !== 'black' && sourceLinks[event.producer]) {
                ...
                if ('transitionLength' in event) {
                    EDLfile +=
                        self.prepadString(event.transitionLength, 3, '0') + ' '
                } else {
                    EDLfile += '    '
                }
...
MltXmlParser.prototype.baseName = function (fileName, keepExtension) {
    ...
    else if (fileName.indexOf('\\') !== -1) name = fileName.split('\\').pop()
